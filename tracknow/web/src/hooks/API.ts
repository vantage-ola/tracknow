...
const endpoints = {
  ...
  PUT_USER_PROFILE_PIC: (user_id: Number) =>
    `${API_PREFIX_URL}/users/${user_id}/profile_picture`,

  POST_GET_LAPTIME: `${API_PREFIX_URL}/user/laptimes`, // user posts a laptime or view all their laptimes jwt*
  GET_MY_LAPTIMES: (page: Number) =>
    `${API_PREFIX_URL}/user/laptimes?page=${page}`, // user view all their laptimes with pagination
  ...
  GET_ONE_LAPTIME: (user_id: number, id: number) =>
    `${API_PREFIX_URL}/users/${user_id}/laptimes/${id}`, // see a specific laptime of someone
  GET_USERS_LAPTIMES: (user_id: number, page: number) =>
    `${API_PREFIX_URL}/users/${user_id}/laptimes?page=${page}`, // see personal moments of other users

  EDIT_USER_LAPTIME: (id: number) =>
    `${API_PREFIX_URL}/user/laptime/edit/${id}`,
  DELETE_USER_LAPTIME: (id: number) =>
    `${API_PREFIX_URL}/user/laptime/delete/${id}`,
...
async function handleLaptimes(
  page: number = 1,
  newLaptime?: Laptime,
): Promise<CreateLaptimeResponse | GetUserLaptimesResponse[]> {
  const token = localStorage.getItem("access_token"); // after logging in, we retrieve the token to get access to some of the functions
...
async function fetchEveryoneLaptime(
  page: number,
): Promise<GetUserLaptimesResponse[]> {
...
async function fetchAUserLaptime(
  user_id: number,
  id: number,
): Promise<GetUserLaptimesResponse> {
  const token = localStorage.getItem("access_token");
...
async function fetchUsersLaptimes(
  user_id: number,
  page: number,
): Promise<GetUserLaptimesResponse[]> {
  const token = localStorage.getItem("access_token");
...
async function editUserLaptime(
  id: number,
  edit_laptime: Laptime,
): Promise<Laptime> {
  const token = localStorage.getItem("access_token");
